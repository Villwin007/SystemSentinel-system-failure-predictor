...
class BiLSTMPredictorFixed:
    def __init__(self):
        ...
        # Store recent metrics for sequence creation - FIXED: Store raw metrics, not features
    ...
    def add_to_sequence_buffer(self, current_metric):
        """Add raw metric to sequence buffer - FIXED VERSION"""
    ...
    def create_features_from_metric(self, metric):
        """Create features from a single metric point"""
        features = {}
        
        # Basic metrics
        base_features = ['cpu_percent', 'memory_percent', 'memory_used_gb', 'disk_usage_percent', 'disk_used_gb']
        for feature in base_features:
            features[feature] = metric[feature]
    ...
    def calculate_rolling_features(self):
        """Calculate rolling features from the sequence buffer"""
        ...
        for feature in base_features:
            if len(df) >= 5:
                # Rolling statistics for the most recent point
                rolling_features[f'{feature}_rolling_mean_5'] = df[feature].rolling(window=5, min_periods=1).mean().iloc[-1]
                rolling_features[f'{feature}_rolling_std_5'] = df[feature].rolling(window=5, min_periods=1).std().iloc[-1]
                rolling_features[f'{feature}_rolling_min_5'] = df[feature].rolling(window=5, min_periods=1).min().iloc[-1]
                rolling_features[f'{feature}_rolling_max_5'] = df[feature].rolling(window=5, min_periods=1).max().iloc[-1]
            
            # Rate of change
            if len(df) >= 2:
                rolling_features[f'{feature}_roc'] = df[feature].pct_change().fillna(0).iloc[-1]
        
        # Interaction features for most recent point
        current = self.sequence_buffer[-1]
        rolling_features['cpu_memory_ratio'] = current['cpu_percent'] / (current['memory_percent'] + 1)
        rolling_features['system_load_index'] = (current['cpu_percent'] * 0.6 + current['memory_percent'] * 0.3 + current['disk_usage_percent'] * 0.1)
        rolling_features['resource_pressure'] = (
            (current['cpu_percent'] > 80) + 
            (current['memory_percent'] > 80) + 
            (current['disk_usage_percent'] > 80)
        )
        
        return rolling_features
    
    def create_sequence_for_prediction(self):
        """Create sequence for BiLSTM prediction - FIXED VERSION"""
        ...
        sequence_features = []
        for i, metric in enumerate(recent_sequence):
            # Create base features for this point
            features = self.create_features_from_metric(metric)
            
            # Add rolling features if we have enough context
            if i >= 4:  # We need at least 5 points for rolling features
                rolling_features = self.calculate_rolling_features()
                # Update the feature vector with rolling features
                for j, feature_name in enumerate(self.feature_names):
                    if feature_name in rolling_features:
                        features[j] = rolling_features[feature_name]
    ...
    def predict_anomaly(self, sequence):
        ...
        try:
            # Scale the sequence
            sequence_flat = sequence.reshape(-1, len(self.feature_names))
    ...
    def analyze_temporal_pattern(self, sequence_scaled):
        ...
        if not patterns:
            ...
            current_cpu = sequence_df['cpu_percent'].iloc[-1]
            current_memory = sequence_df['memory_percent'].iloc[-1]
            
            if current_cpu > 90:
                patterns.append("High CPU load")
...
"""
    ...
    def start_bilstm_monitoring(self):
        """Start real-time monitoring with BiLSTM - FIXED VERSION"""
            ...
            for message in pubsub.listen():
                if message['type'] == 'message':
                    try:
                        ...
                        # FIXED: Add to sequence buffer first
                        self.add_to_sequence_buffer(data)
...
if __name__ == '__main__':
    predictor = BiLSTMPredictorFixed()
